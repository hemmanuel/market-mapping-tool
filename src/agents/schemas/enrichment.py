...
class CompanyEnrichmentTaxonomy(BaseModel):
...
class CompanyEnrichmentVCDossier(BaseModel):
    model_config = ConfigDict(extra="ignore")
...
class CompanyEnrichmentFounder(BaseModel):
    model_config = ConfigDict(extra="ignore")
...
class CompanyEnrichmentUnitEconomicsInference(BaseModel):
    model_config = ConfigDict(extra="ignore")

    acv_proxy: Optional[str] = None
    retention_quality: Optional[str] = None
    distribution_friction: Optional[str] = None


class CompanyEnrichmentStrategicAnalysis(BaseModel):
    model_config = ConfigDict(extra="ignore")

    market_depth_score: Optional[float] = None
    market_narrative: Optional[str] = None
    competitive_noise_level: Optional[str] = None
    unit_economics_inference: CompanyEnrichmentUnitEconomicsInference = Field(
        default_factory=CompanyEnrichmentUnitEconomicsInference
    )
    ai_survival_score: Optional[float] = None
    ai_force_multiplier_thesis: Optional[str] = None


class CompanyEnrichmentMetricRationales(BaseModel):
    model_config = ConfigDict(extra="ignore")

    market_scale_rationale: Optional[str] = None
    competition_rationale: Optional[str] = None
    contract_size_rationale: Optional[str] = None
    stickiness_rationale: Optional[str] = None
    sales_difficulty_rationale: Optional[str] = None
    ai_defensibility_rationale: Optional[str] = None
...
class CompanyEnrichment(BaseModel):
    ...
    tangibility_score: Optional[int] = None
    customer_type: str
    investment_thesis_one_liner: str
    dimension_scores: dict[str, Optional[float]] = Field(default_factory=dict)
    venture_scale_score: Optional[float] = None
    stage_estimate: str
    rationale: str
    taxonomy: CompanyEnrichmentTaxonomy = Field(default_factory=CompanyEnrichmentTaxonomy)
    vc_dossier: CompanyEnrichmentVCDossier = Field(default_factory=CompanyEnrichmentVCDossier)
    founders: list[CompanyEnrichmentFounder] = Field(default_factory=list)
    company_twitter_url: Optional[str] = None
    strategic_analysis: CompanyEnrichmentStrategicAnalysis = Field(
        default_factory=CompanyEnrichmentStrategicAnalysis
    )
    metric_rationales: CompanyEnrichmentMetricRationales = Field(
        default_factory=CompanyEnrichmentMetricRationales
    )
